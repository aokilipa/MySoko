...
public class MainActivityTest {
    ...
    public void listOfRibotsShows() {
        List<Product> testDataProducts = TestDataFactory.makeListRibots(20);
        when(component.getMockDataManager().getRibots())
                .thenReturn(Observable.just(testDataProducts));
        ...
        for (Product product : testDataProducts) {
            ...
            String name = String.format("%s %s", product.profile().name().first(),
                    product.profile().name().last());
            onView(withText(name))
                    .check(matches(isDisplayed()));
            onView(withText(product.profile().email()))
                    .check(matches(isDisplayed()));
            position++;
        }
    }
